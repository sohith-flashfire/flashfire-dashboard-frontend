...
export default function AdminDashboard({
    ...
    const [activeTab, setActiveTab] = useState<
        "overview" | "users" | "history" | "sessions"
    >("overview");
    ...
    const handleDeleteUser = async (username: string) => {
        ...
        try {
            const response = await authFetch(
                `/api/admin/delete-user?username=${encodeURIComponent(
                    username
                )}`,
                {
                    method: "DELETE",
    ...
    const handleGenerateSessionKey = async () => {
        try {
            const response = await authFetch(
                "/api/sessions/generate-session-key",
                {
                    method: "POST",
                    body: JSON.stringify({
    ...
    const handleRevokeUserSessions = async (username: string) => {
        if (
            !confirm(
                `Are you sure you want to revoke all sessions for ${username}?`
            )
        )
            return;
        ...
        try {
            const response = await authFetch(
                "/api/sessions/revoke-user-sessions",
                {
                    method: "POST",
    ...
    const handleDeleteUserAccount = async (username: string) => {
        if (
            !confirm(
                `Are you sure you want to delete ${username}'s account? This will log them out from all devices.`
            )
        )
            return;
                        ...
                        <div className="flex items-center space-x-4">
                            ...
                            <div>
                                ...
                                <p className="text-gray-600 mt-1">
                                    Advanced system management & analytics
                                </p>
                            </div>
                        </div>
                        <div className="flex items-center space-x-4">
                        ...
                        {activeTab === "sessions" && (
                            <div className="space-y-6">
                                <div className="flex flex-col sm:flex-row sm:items-center sm:justify-between space-y-4 sm:space-y-0">
                                    ...
                                    <button
                                        onClick={() =>
                                            setShowGenerateSessionKey(true)
                                        }
                                        className="px-6 py-3 bg-gradient-to-r from-green-500 to-emerald-600 text-white rounded-2xl hover:from-green-600 hover:to-emerald-700 transition-all duration-300 shadow-lg hover:shadow-xl flex items-center space-x-2"
                                    >
                            ...
                            <div className="space-y-6">
                                ...
                                {(loginHistory?.length || 0) === 0 && (
                                    <div className="text-center py-12">
                                        ...
                                        <p className="text-gray-600">
                                            Login events will appear here as
                                            they occur.
                                        </p>
                                    </div>
                                )}
