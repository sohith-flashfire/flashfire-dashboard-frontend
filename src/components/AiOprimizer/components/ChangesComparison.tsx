...
export const ChangesComparison: React.FC<ChangesComparisonProps> = ({
  ...
  const getChangedItems = (originalArr: any[], optimizedArr: any[], keyFields: string[]) => {
    return originalArr
      .map((orig, idx) => {
        const opt = optimizedArr[idx];
        if (!opt) return null;
        for (const key of keyFields) {
          if (JSON.stringify(orig[key]) !== JSON.stringify(opt[key])) {
  ...
  const personalInfoFields = [
            'name', 'title', 'email', 'phone', 'location', 'linkedin', 'portfolio', 'github'
  ];
  const personalInfoChanged = personalInfoFields.some(field => originalData.personalInfo[field] !== optimizedData.personalInfo[field]);
  ...
  const changedWorkExperience = getChangedItems(
    ...
    ['position', 'company', 'duration', 'location', 'roleType', 'responsibilities']
  );
  ...
  return (
    <div className="space-y-8 p-6 bg-gray-50 rounded-lg">
      <h2 className="text-2xl font-bold text-gray-800 mb-6">📊 Optimization Changes Summary</h2>
          ...
          <div className="grid grid-cols-1 lg:grid-cols-2 gap-6">
            <div>
              ...
              <div className="text-sm text-gray-700 bg-red-50 p-4 rounded border-l-4 border-red-400 leading-relaxed space-y-2">
                {personalInfoFields.map(field => (
                  originalData.personalInfo[field] !== optimizedData.personalInfo[field] && (
                    <div key={field}><strong>{field.charAt(0).toUpperCase() + field.slice(1)}:</strong> {originalData.personalInfo[field]}</div>
                  )
                ))}
              </div>
            </div>
            <div>
              ...
              <div className="text-sm text-gray-700 bg-green-50 p-4 rounded border-l-4 border-green-400 leading-relaxed space-y-2">
                {personalInfoFields.map(field => (
                  originalData.personalInfo[field] !== optimizedData.personalInfo[field] && (
                    <div key={field}><strong>{field.charAt(0).toUpperCase() + field.slice(1)}:</strong> {optimizedData.personalInfo[field]}</div>
                  )
                ))}
              </div>
            </div>
      ...
      {changedWorkExperience.length > 0 && (
        <div className="bg-white p-6 rounded-lg shadow-sm border">
          ...
          {changedWorkExperience.map(({ original, optimized }, idx) => (
            <div key={original.id} className="mb-8 border border-gray-200 rounded-lg overflow-hidden">
              ...
              <div className="grid grid-cols-1 lg:grid-cols-2 gap-0">
                <div className="p-4 bg-red-25">
                  ...
                  <div className="space-y-2">
                    {original.responsibilities.map((resp, respIndex) => (
                      (!optimized.responsibilities[respIndex] || resp !== optimized.responsibilities[respIndex]) && resp.trim() && (
                        <div key={respIndex} className="flex items-start gap-2">
                          <span className="text-red-600 mt-1">•</span>
                          <div className="text-sm text-gray-700 leading-relaxed">{resp}</div>
                        </div>
                      )
                    ))}
                  </div>
                ...
                <div className="p-4 bg-green-25 border-l border-gray-200">
                  ...
                  <div className="space-y-2">
                    {optimized.responsibilities.map((resp, respIndex) => (
                      (!original.responsibilities[respIndex] || resp !== original.responsibilities[respIndex]) && resp.trim() && (
                        <div key={respIndex} className="flex items-start gap-2">
            ...
            <div>
              ...
              <div className="space-y-3">
                {changedProjects.map(({ original }, idx) => (
                  <div key={original.id} className="bg-red-50 p-4 rounded border-l-4 border-red-400">
                    <div className="font-medium text-sm text-gray-800 mb-2">
                      {original.position} - {original.company}
                    </div>
                    <div className="text-xs text-gray-600 mb-2">
                      {original.duration} | {original.location}
                    </div>
                    <div className="space-y-1">
                      {original.responsibilities.map((resp, respIndex) => (
                        (!changedProjects[idx].optimized.responsibilities[respIndex] || resp !== changedProjects[idx].optimized.responsibilities[respIndex]) && resp.trim() && (
                          <div key={respIndex} className="flex items-start gap-2">
                            <span className="text-red-600 mt-1 text-xs">•</span>
                            <div className="text-xs text-gray-700 leading-relaxed">{resp}</div>
                          </div>
                        )
                      ))}
                    </div>
            ...
            <div>
              ...
              <div className="space-y-3">
                {changedProjects.map(({ optimized }, idx) => (
                  <div key={optimized.id} className="bg-green-50 p-4 rounded border-l-4 border-green-400">
                    <div className="font-medium text-sm text-gray-800 mb-2">
                      {optimized.position} - {optimized.company}
                    </div>
                    <div className="text-xs text-gray-600 mb-2">
                      {optimized.duration} | {optimized.location}
                    </div>
                    <div className="space-y-1">
                      {optimized.responsibilities.map((resp, respIndex) => (
                        (!changedProjects[idx].original.responsibilities[respIndex] || resp !== changedProjects[idx].original.responsibilities[respIndex]) && resp.trim() && (
                          <div key={respIndex} className="flex items-start gap-2">
            ...
            <div>
              ...
              <div className="space-y-3">
                {changedLeadership.map(({ original }) => (
                  <div key={original.id} className="bg-red-50 p-3 rounded border-l-4 border-red-400">
            ...
            <div>
              ...
              <div className="space-y-3">
                {changedLeadership.map(({ optimized }) => (
                  <div key={optimized.id} className="bg-green-50 p-3 rounded border-l-4 border-green-400">
                    <div className="font-medium text-sm text-gray-800 mb-1">
                      {optimized.title}
                    </div>
                    <div className="text-sm text-gray-700">
            ...
            <div>
              ...
              <div className="space-y-3">
                {changedEducation.map(({ original }) => (
                  <div key={original.id} className="bg-red-50 p-4 rounded border-l-4 border-red-400">
                    <div className="font-medium text-sm text-gray-800 mb-2">
                      {original.institution} - {original.degree}, {original.field}
                    </div>
            ...
            <div>
              ...
              <div className="space-y-3">
                {changedEducation.map(({ optimized }) => (
                  <div key={optimized.id} className="bg-green-50 p-4 rounded border-l-4 border-green-400">
                    <div className="font-medium text-sm text-gray-800 mb-2">
                      {optimized.institution} - {optimized.degree}, {optimized.field}
                    </div>
      ...
      {/* Summary Note */}
      {(personalInfoChanged || summaryChanged || changedWorkExperience.length > 0 || changedProjects.length > 0 || changedLeadership.length > 0 || changedSkills.length > 0 || changedEducation.length > 0) && (
        <div className="bg-blue-50 p-6 rounded-lg border border-blue-200">
          <div className="flex items-center justify-center text-blue-800">
            <span className="text-2xl mr-3">ℹ️</span>
            <span className="font-medium">Only changed sections and content are shown above.</span>
          </div>
        </div>
      )}
    </div>
